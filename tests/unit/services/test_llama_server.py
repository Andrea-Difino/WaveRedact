...
class TestLlamaServerService:
    ...
    @patch(f"{MODULE_PATH}.LlamaServerService._find_executable")
    @patch(f"{MODULE_PATH}.urllib.request.urlretrieve")
    def test_init_skips_download_if_executable_exists(self, mock_urlretrieve, mock_find_exe, mock_atexit):
        mock_find_exe.return_value = "/fake/path/llama-server.exe"

        service = LlamaServerService("fake_model.gguf")

        mock_urlretrieve.assert_not_called()
        assert service.exe_path == "/fake/path/llama-server.exe"
    ...
    def test_init_downloads_and_extracts_if_missing(
        self, mock_makedirs, mock_remove, mock_zip, mock_urlretrieve, mock_find_exe, mock_atexit
    ):
        mock_find_exe.side_effect = [None, "/fake/path/llama-server.exe"]

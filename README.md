...
| **Validation LLM** (Optional) | `Qwen2.5-7B-Instruct-Q4_K_M.gguf` (> 8GB RAM) <br> `gemma-3-4b-it-UD-Q5_K_XL.gguf` (<= 8GB RAM) | ~4.3 GB (Qwen) <br> ~2.64 GB (Gemma) | CPU or GPU (via `llama.cpp` auto-offload) |
...
- **<= 8 GB RAM:** Loads a smaller model (`Gemma-3-4b`, Q5_K_XL, ~2.64 GB) for stability.
